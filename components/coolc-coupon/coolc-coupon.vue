<template>
    ...
    <view class="coupon-item">
		...
		<view class="get-btn" v-if="types" :style="{color:color, borderColor:color, background:solid}" @click="oneck(item)">{{cktit}}</view>
		<view  class="get-btn" v-if="!types" :style="{color:color, borderColor:color, background:solid}" @click="oneck(item)">{{cktit}}</view>
    </view>
</template>
...
.coupon-item {
	...
	.coupon-money {
		...
		.tit { width:100%; height:auto; line-height:50upx; font-size:$font-sm; color:$font-color-999; }
		.demand { width:100%; height:auto; line-height:30upx; font-size:$font-sm; color:$font-color-999; }
